...
def editService(id, val1, val2, val3, val4, val5, val6):
    ...
    patentSFEntry.insert(0, val1)
    brandSFEntry.insert(0, val2)
    modelSFEntry.insert(0, val3)
    nameSFEntry.insert(0, val4)
    dateEntry.insert(0, val5)
    descriptionEntry.insert(1.0, val6)
    ...
    def saveNewEntry():
        ...
        if (len(val1) > 0) and (len(val2) > 0) and (len(val3) > 0) and (len(val5) > 0) and (len(val6) > 0):
            ...
            if(len(name) == 0):
                name = "NO AGREGADO"
            validationForEdit(showEditC, type, id, val1, val2, val3, name,val5, val6)
        else:
