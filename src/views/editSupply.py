from customtkinter import CTkImage, CTkLabel, CTkEntry, CTkFrame, CTkToplevel, CTkButton
from PIL import Image

from ..components.enterpriseLogo import enterpriseLogo
from ..components.MBox import MBox

from dbconection.validationEditEntry import validationForEdit

def editSupply(id, val1, val2, val3, val4, val5, val6):
    print(id)
    ...
    nameSupplyEntry.insert(0, val1)
    priceUnityEntry.insert(0, val2)
    priceSaleEntry.insert(0, val3)
    revenueEntry.insert(0, val4)
    inversionEntry.insert(0, val5)
    stockEntry.insert(0, val6)

    def saveNewEntry():
        type = "SUPPLY"
        newVal1 = nameSupplyEntry.get()
        newVal2 = priceUnityEntry.get()
        newVal3 = priceSaleEntry.get()
        newVal4 = revenueEntry.get()
        newVal5 = inversionEntry.get()
        newVal6 = stockEntry.get()
        if (len(newVal1) > 0) and (len(newVal2) > 0) and (len(newVal3) > 0) and (len(newVal5) > 0) and (len(newVal6) > 0):
            name=val4
            if(len(name) == 0):
                name = "NONE"
            validationForEdit(showEditC, type, id, newVal1, newVal2, newVal3, newVal4,newVal5, newVal6)
        else:
